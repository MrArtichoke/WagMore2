...
db.define_table('profile',
                ...
                Field('commonPawCoin', 'integer', default=5),   #rewards: (CC = common coin, UC = uncommon coin, RC = rare coin)
                ...
                Field('uncommonPawCoin', 'integer', default=2),
                Field('dateLastLoggedIn', 'date', default = request.now, requires = IS_DATE(format=('%Y-%m-%d'))),
...
db.profile.dateLastLoggedIn.writable = False
db.profile.dateLoggedin.writable = False
db.profile.commonPawCoin.writable = False
...
db.votes.voter.requires = IS_IN_DB(db,db.forum_post.user_id)
